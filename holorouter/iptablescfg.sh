...
iptables -A FORWARD -p tcp -d 10.0.0.11 --dport 22 -j ACCEPT
iptables -A FORWARD -p tcp -s 10.0.0.11 --sport 22 -j ACCEPT

# DNAT/SNAT ssh on the Manager over external port 5480
iptables -A PREROUTING -t nat -d 192.168.100.2 -p tcp --dport 5480 -j DNAT --to-d 10.0.0.11:22
iptables -A POSTROUTING -t nat -p tcp -d 10.0.0.11 --dport 22 -j SNAT --to-source 192.168.100.2:5480
...
iptables -A FORWARD -p tcp -s 10.0.0.2 --sport 22 -j ACCEPT

# DNAT/SNAT port forward ssh to the Main Console
iptables -A PREROUTING -t nat -p tcp -d 192.168.100.2 --dport 22 -j DNAT --to 10.0.0.2:22
iptables -A POSTROUTING -t nat -p tcp -d 10.0.0.2 --dport 22 -j SNAT --to-source 192.168.100.2

# allow 5901 on the Main Console
iptables -A FORWARD -p tcp -d 10.0.0.2 --dport 5901 -j ACCEPT
iptables -A FORWARD -p tcp -s 10.0.0.2 --sport 5901 -j ACCEPT

# DNAT/SNAT port forward screen sharing over 5901 to the Main Console
iptables -A PREROUTING -t nat -p tcp -d 192.168.100.2 --dport 5901 -j DNAT --to 10.0.0.2:5901
iptables -A POSTROUTING -t nat -p tcp -d 10.0.0.2 --dport 5901 -j SNAT --to-source 192.168.100.2

# allow RDP 3389 on the Main Console
iptables -A FORWARD -p tcp -d 10.0.0.2 --dport 3389 -j ACCEPT
iptables -A FORWARD -p tcp -s 10.0.0.2 --sport 3389 -j ACCEPT

# DNAT/SNAT port forward 3389 for RDP to the Main Console
iptables -A PREROUTING -t nat -p tcp -d 192.168.100.2 --dport 3389 -j DNAT --to 10.0.0.2:3389
iptables -A POSTROUTING -t nat -p tcp -d 10.0.0.2 --dport 3389 -j SNAT --to-source 192.168.100.2
