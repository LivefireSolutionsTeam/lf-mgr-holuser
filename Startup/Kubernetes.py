# Kubernetes.py - version v1.7 - 02-February 2024
import datetime
import os
import sys
import io
...
def checkKubernetes_certs(entry):
    ...
    :param entry: from Resources/Kubernetes.txt, e.g. primary host:privileged account:password:renewal command
    """
    renew = False
    ...
    (remotehost, account, passwd, renewcommand) = entry.split(':')
    # print(f'{remotehost} {account} {passwd} {renewcommand}')
    ...
    with io.open(checkscriptfile, "w", newline='\n') as lf: 
        lf.write(f'{checkcmd}\n')
    lf.close()
    lsf.scp(checkscriptfile, f'{account}@{remotehost}:{scriptname}', passwd)
    output = lsf.ssh(f'/bin/bash ./{scriptname} 2>&1', f'{account}@{remotehost}', passwd)
    ...
    if renew:
        ...
        try:
          ...
          output = lsf.ssh(renewcommand, f'{account}@{remotehost}', passwd)
